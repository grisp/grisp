...
device_request(Bus, Register, Delay, BytesToRead) ->
    [ok] = grisp_ni2c:transfer(Bus, [{?DEVICE_ADR, 0, <<Register:8>>}]),
    timer:sleep(Delay),
    [Response] = grisp_ni2c:transfer(Bus, [{?DEVICE_ADR, 1, BytesToRead}]),
    % Response = grisp_i2c:msgs([?DEVICE_ADR, {write, <<Register:8>>},
    %                            {sleep, Delay},
    %                            {read, BytesToRead}]),
    {ok, Response}.
